        ...
        <div class="flex justify-between h-20">
            <div class="flex">
                <div class="shrink-0 flex items-center">
                    <a href="{{ Auth::user()->role === 'admin' ? route('admin.dashboard') : route('customer.catalog') }}" class="flex items-center group gap-3">
                        <div class="relative">
                            @if (config('app.logo_url'))
                                <img src="{{ config('app.logo_url') }}" class="h-10 w-10 rounded-full object-cover border-2 border-white shadow-sm group-hover:scale-110 transition duration-300" alt="Logo">
                            @else
                                <div class="bg-blue-600 p-2 rounded-full shadow-lg shadow-blue-200 group-hover:scale-110 transition duration-300">
                        ...
                        <div class="flex flex-col">
                            <span class="font-black text-lg text-gray-900 tracking-tighter leading-none uppercase">
                                Renesca
                            </span>
                            <span class="text-[10px] font-black text-blue-600 uppercase tracking-[0.2em] leading-none mt-1">
                                Aquatic
                            </span>
                        </div>
                    </a>
                ...
                <div class="hidden space-x-2 sm:-my-px sm:ms-10 sm:flex items-center">
                    ...
                    @if(Auth::user()->role === 'admin')
                        <div class="relative" x-data="{ openAdmin: false }">
                            ...
                            <div x-show="openAdmin" @click.away="openAdmin = false" x-transition class="absolute left-0 mt-2 w-48 bg-white border border-gray-100 rounded-[1.5rem] shadow-xl p-2 z-[110]">
                    ...
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="flex items-center gap-3 p-1 pr-3 bg-gray-50 rounded-2xl hover:bg-gray-100 transition border border-transparent hover:border-gray-200">
                                <img class="h-9 w-9 rounded-full object-cover border-2 border-white shadow-sm" src="{{ asset('storage/'.Auth::user()->profile_photo_path) }}" alt="{{ Auth::user()->name }}" />
